...
> ⚠️ The Jetson reaches the Ground Station over the Orin Nano's USB-gadget network (`jetson 192.168.55.1`, `laptop 192.168.55.100`). Make sure the computer holds that address so the Jetson's default `GS_HOST` resolves — or override `GS_HOST` on the Jetson side.
